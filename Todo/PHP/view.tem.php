...
<body>
    <div class="container">
        <div class="my-4">
            <!-- 現在のユーザー名表示 -->
            <p>現在のユーザー名: <?= htmlspecialchars($_SESSION['user_name'], ENT_QUOTES, 'UTF-8') ?></p>
        ...
        <form action="" method="GET" class="">
            <div class="col-md-6">
                <input type="text" class="form-control search_inputs" placeholder="検索するキーワード" name="keyword"
        ...
        <!-- ユーザーの役割によって表示を変更 -->
        <?php if ($_SESSION['role'] === 'public' || $_SESSION['role'] === 'private'): ?>
            <div class="input-area mb-4">
                <form action="mysql.php" method="POST" class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="title" name="title" placeholder="TODOのタイトル"
                            maxlength="255" required>
                    </div>
                    <div class="col-md-6">
                        <label for="value" class="form-label">TODOの内容</label>
                        <input type="text" class="form-control" id="value" name="value" placeholder="TODOの内容"
                            maxlength="255" required>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="progress_number" name="progress_number" min="0"
                            max="100" required>
                        <input type="range" class="form-range" id="progress" name="progress" value="0" min="0" max="100"
                            data-unit="%" disabled>
                    </div>
                    <div class="col-md-6 d-flex align-items-end">
                        <button type="submit" class="btn btnshine btn-primary btn-sm" name="action" value="add">追加</button>
                        <input type="hidden" name="user_id" value="<?= $_SESSION['user_id'] ?>">
                        <input type="hidden" name="user_role" value="<?= $_SESSION['role'] ?>">
                    </div>
                </form>
            </div>
            ...
            <?php
                ...
                try {
                    ...
                    // SQL文の準備
                    $sql = "SELECT * FROM todo WHERE user_id = :user_id AND status = '未完了' ORDER BY due_date ASC";
            ...
            <?php
                try {
                    ...
                    // SQLの準備
                    $sql = "SELECT * FROM todo WHERE user_id = :user_id AND status = '完了' ORDER BY due_date ASC";
            ...
            <?php
                ...
                try {
                    ...
                    $sql = "SELECT todo.*, users.user_name
                            ...
                            WHERE todo.status = '未完了' AND todo.user_role = 'public'
            ...
            <?php
                ...
                try {
                    ...
                    $sql = "SELECT todo.*, users.user_name
                            FROM todo
                            LEFT JOIN users ON todo.user_id = users.user_id
                            WHERE todo.status = '完了' AND todo.user_role = 'public'
                            ORDER BY todo.due_date ASC";
