...
const onClickSquare = (index) => {
  ...
  if (stoneStateList.filter(state => state === 1).length === 0 || stoneStateList.filter(state => state === 2).length === 0) {
    ...
    judgeElement.text(`ゲーム終了です。白${whiteStonesNum}、黒${blackStonesNum}で、${winnerText}`);
    // "endless" というIDを持つボタン要素を取得
    const endlessButton = $("#endless");
    // endlessボタンのdisabled属性を解除
    endlessButton.removeAttr("disabled");
  }
...
$("#endless").on("click", () => {
  ...
  $("#endless").attr("disabled", "disabled");
  // .judge クラスを持つ要素を取得
  const judgeElement = $(".judge");
...
$(document).ready(() => {
  ...
  passButton.on("click", changeTurn)
});
